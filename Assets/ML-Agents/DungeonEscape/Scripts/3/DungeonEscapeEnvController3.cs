...
public class DungeonEscapeEnvController3 : MonoBehaviour
{
    // 玩家信息类，存储玩家代理及其初始状态和组件。
    [System.Serializable]
    public class PlayerInfo
    {
        public PushAgentEscape3 Agent; // 玩家代理对象
    ...
    {
        public SimpleNPC3 Agent; // 敌人代理对象
        [HideInInspector] public Vector3 StartingPos; // 初始位置
        [HideInInspector] public Quaternion StartingRot; // 初始旋转
        [HideInInspector] public Rigidbody Rb; // 刚体组件
        [HideInInspector] public Collider Col; // 碰撞器组件
    ...
    // 玩家字典，用于快速查找玩家信息
    private Dictionary<PushAgentEscape3, PlayerInfo> m_PlayerDict = new Dictionary<PushAgentEscape3, PlayerInfo>();
    ...
    public GameObject Switch1;//按钮对象
    public GameObject Switch2;//按钮对象
    private SimpleMultiAgentGroup m_AgentGroup; // 多智能体组
    public bool Switch1Triggered=false;
    public bool Switch2Triggered=false;
    void Start()
    {
        // 获取地面边界
        ...
        // 记录敌人的初始信息
        for (int i = 0; i < DragonsList.Count; i++)
        {
            if (i != DragonsList.Count - 1)
            {
                DragonsList[i].StartingPos = DragonsList[i].Agent.transform.position;
                DragonsList[i].StartingRot = DragonsList[i].Agent.transform.rotation;
                DragonsList[i].T = DragonsList[i].Agent.transform;
                DragonsList[i].Col = DragonsList[i].Agent.GetComponent<Collider>();
            }
        }
    ...
    // 当玩家碰到危险区域时调用
    public void TouchedHazard(PushAgentEscape3 agent)
    {
    ...
    // 当玩家被敌人吃掉时调用
    public void KilledByBaddie(PushAgentEscape3 agent, Collision baddieCol)
    {
    ...
    public void CreateDragon()
    {
        ...
        // 激活敌人对象，使其重新参与本轮训练
        DragonsList[DragonsList.Count-1].Agent.gameObject.SetActive(true);

    }
    public void CreateSwitch1()
    {
        Switch1.transform.SetPositionAndRotation(new Vector3(GetRandomSpawnPos().x,0,GetRandomSpawnPos().z),GetRandomRot());
        Switch1.SetActive(true);
    }
    public void CreateSwitch2()
    {
        Switch2.transform.SetPositionAndRotation(new Vector3(GetRandomSpawnPos().x,0,GetRandomSpawnPos().z),GetRandomRot());
    ...
    void ResetScene()
    {
        ...
        // 遍历所有敌人（龙）信息，重置除了最后一只龙以外每个敌人的状态
        for(int i=0;i<DragonsList.Count;i++)
        {
            if (i != DragonsList.Count - 1)
            {
                ...
                // 将敌人的位置和旋转重置为初始值
                DragonsList[i].Agent.transform.SetPositionAndRotation(DragonsList[i].StartingPos, DragonsList[i].StartingRot);

                // 设置敌人的随机行走速度，增加训练多样性
                DragonsList[i].Agent.SetRandomWalkSpeed();
