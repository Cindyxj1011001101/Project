...
public class PushAgentEscape3 : Agent
{
    // 拾取到的钥匙对象，在拾取后激活显示。
    ...
    private DungeonEscapeEnvController3 m_GameController;

    // 初始化方法，在游戏开始时调用一次。
    public override void Initialize()
    {
        // 获取父物体上的环境控制器组件。
        m_GameController = GetComponentInParent<DungeonEscapeEnvController3>();
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        //如果碰到按钮，出现钥匙
        if (col.transform.CompareTag("switch1"))
        {
            Debug.Log("Create Key");
            m_GameController.CreateKey(); //出现钥匙
        }
        if (col.transform.CompareTag("switch2"))
        {
            Debug.Log("Create Key");
            m_GameController.CreateDragon(); //出现钥匙
        }
    }
